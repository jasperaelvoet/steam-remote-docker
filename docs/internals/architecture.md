...
One container, one session, one supported path. The image is an immutable Arch
Linux root with a single entrypoint script,
...
`a required process exited` and returns nonzero, stopping the container. There
is deliberately no in-place recovery or partial restart — the restart policy
restarts the whole session, which is the only state the design supports. The
lifecycle supervisor is the one *auxiliary* process: it can die without taking
the session down (health degrades instead).
...
mounts provide `/run`, `/tmp`, `/var/tmp`, and `/var/lib/xkb`. The split is an
...
The Xwayland instances are launched through a generated wrapper that passes an
explicit `-dpi` matching the advertised physical display size — Xwayland
...
Audio is a virtual 8-channel 48kHz null sink (`Steam_Stream_Audio`) set as the
default sink. Games play into it, Steam captures it for the stream; there is
no host audio device.

The Gamescope build itself is patched for this headless streaming use case —
see [Streaming pipeline](/internals/streaming).

## The lifecycle supervisor

The supervisor is a once-per-second loop in the entrypoint script that drives
the [idle lifecycle](/guide/idle-lifecycle). Each tick it:

1. **Detects streaming** — established TCP connections on `27036`, plus the
   state of Gamescope's PipeWire `Video/Source` node (`pw-cli`).
2. **Detects games** — scans `/proc/*/environ` for `SteamAppId`/`SteamGameId`.
...
4. **Combines** the three signals: any `unknown` wins over `true`, `true` over
   `false` — uncertainty always fails toward full rate.
...
2. **cursor-shim-builder** — compiles the cursor-normalization shim for both
   x86-64 and i386 (Steam is 32-bit in places), so `ld.so` can pick the right
   ABI via `$LIB`.
3. **Runtime** — Arch Linux with Steam from multilib, Mesa/RADV and VA-API for
   AMD, PipeWire, and the patched Gamescope installed over the stock package.
   The shim is registered in `/etc/ld.so.preload` (Steam's launcher clears
   `LD_PRELOAD`, so a preload file is the only reliable hook).

CI (`.github/workflows/container.yml`) validates sources with `bun run check`
and ShellCheck, then builds and publishes `linux/amd64` to
`ghcr.io/jasperaelvoet/steam-remote-docker` on every push to `main`.
